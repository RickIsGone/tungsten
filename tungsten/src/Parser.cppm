...
namespace tungsten {
   ...
   private:
      ...
      std::unique_ptr<StructAST> _parseStruct();
      ...
      std::vector < std::unique_ptr<StructAST> _structs;
      ...
      while (_peek().type != TokenType::EndOFFile) {
         switch (_peek().type) {
            ...
            case TokenType::Struct:
               _structs.push_back(_parseStruct());
               break;
   ...
   // }
   std::unique_ptr<StructAST> Parser::_parseStruct() {
      const Token token = _peek();
      _consume(); // consume struct
      std::string name = _parseIdentifierName(true);
      if (name.empty())
         return _logError<StructAST>("expected a struct name");

      if (_peek().type != TokenType::OpenBrace)
         return _logError<StructAST>("expected '{' after " + name);
      _consume(); // consume {
      std::vector<std::unique_ptr<ExpressionAST>> variables;
      ...
      while (_peek().type != TokenType::CloseBrace && _peek().type != TokenType::EndOFFile) {
         if (_peek().type == TokenType::Fun) {
            if (!hasErrors)
               _logError<StructAST>("structs cannot contain functions");
            _parseFunctionDeclaration(); // parsing to skip
            hasErrors = true;
         } else {
         }
      }
