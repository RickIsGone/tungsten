...
export namespace tungsten {
   ...
   llvm::Value* loadIfPointer(llvm::Value* val, llvm::Type* ty, const std::string& name) {
      if (val->getType()->isPointerTy())
         return Builder->CreateLoad(ty, val, name + ".load");
      return val;
   }
   llvm::Value* loadIfPointer(llvm::Value* val, llvm::Type* ty) {
      if (val->getType()->isPointerTy())
         return Builder->CreateLoad(ty, val, val->getName() + ".load");
      return val;
   }
   ...
   llvm::Value* loadDereferenceAddressIfUnaryStar(ExpressionAST* expr, llvm::Value* exprValue, const std::string& name) {
      ...
      return Builder->CreateLoad(pointerValueType, exprValue, name + ".deref");
   }
   ...
   llvm::Value* BinaryExpressionAST::codegen() {
      ...
      if (_RHS->astType() == ASTType::UnaryExpression && static_cast<UnaryExpressionAST*>(_RHS.get())->op() == "*"sv) {
         ...
         RHS = Builder->CreateLoad(rhsValueType, RHS, RHS->getName() + ".derefr");
      }
      ...
      if (isAssignmentOp) {
         ...
         if (_LHS->astType() == ASTType::IndexAccessExpression) {
            ...
            if (indexVal->getType()->isPointerTy())
               indexVal = Builder->CreateLoad(expressionValueTypeForCodegen(indexExpr->index().get()), indexVal, indexVal->getName() + ".idx.load");
            ...
            if (baseTy->isArrayTy()) {
               ...
               if (indexExpr->array()->astType() == ASTType::VariableExpression && arrayPtr->getType()->isPointerTy() && !llvm::isa<llvm::Argument>(arrayPtr))
                  arrayPtr = Builder->CreateLoad(arrayPtr->getType(), arrayPtr, arrayPtr->getName() + ".arrptr.load");
         ...
         if (_op == "="sv) {
            ...
            if (_RHS->astType() != ASTType::IndexAccessExpression && loadedR->getType()->isPointerTy())
               loadedR = Builder->CreateLoad(rhsValueType, loadedR, loadedR->getName() + ".load");
            loadedR = castToCommonType(loadedR, lhsValueType);
         } else {
            if (LHS->getType()->isPointerTy())
               loadedL = Builder->CreateLoad(lhsValueType, LHS);
            else
               loadedL = LHS;
      ...
      if (_init) {
         ...
         if (_init->astType() == ASTType::NullPtr && type->isArrayTy()) {
            ...
            if (_init->astType() == ASTType::UnaryExpression && static_cast<UnaryExpressionAST*>(_init.get())->op() == "*"sv) {
               ...
               if (!initVal)
                  return LogErrorV("invalid dereference initializer");
               initVal = Builder->CreateLoad(type, initVal, initVal->getName() + ".derefl");
            }
            ...
            if (initVal->getType()->isPointerTy() && !type->isPointerTy()) {
               ...
               if (!initValueType)
                  initValueType = type;
               initVal = Builder->CreateLoad(initValueType, initVal, initVal->getName() + ".load");
            }
   ...
   llvm::Value* MemberAccessAST::codegen() {
      ...
      switch (_member->astType()) {
         case ASTType::VariableExpression: {
            ...
            switch (baseTy->kind()) {
               case TypeKind::Pointer:
                  type = static_cast<PointerTy*>(baseTy.get())->pointee();
                  break;
               case TypeKind::Reference:
                  type = static_cast<ReferenceTy*>(baseTy.get())->reference();
                  break;
               case TypeKind::Class:
                  type = baseTy;
                  break;
               default:
                  return LogErrorV("member access base must be a pointer, reference, or class type");
            }
            ...
            if (_base->type()->kind() != TypeKind::Pointer && _base->type()->kind() != TypeKind::Reference && !base->getType()->isPointerTy()) {
               llvm::AllocaInst* tmp = Builder->CreateAlloca(type->llvmType(), nullptr, base->getName() + ".ptr");
               Builder->CreateStore(base, tmp);
               base = tmp;
            }
         ...
         case ASTType::CallExpression: {
            ...
            if (function->getName() != staticFunName) {
               if (_base->type()->kind() != TypeKind::Pointer && _base->type()->kind() != TypeKind::Reference && !base->getType()->isPointerTy()) {
                  llvm::AllocaInst* tmp = Builder->CreateAlloca(_base->type()->llvmType(), nullptr, base->getName() + ".ptr");
                  Builder->CreateStore(base, tmp);
                  base = tmp;
               }
               args.push_back(base);
            ...
            for (size_t i = 0; i < callExpr->args().size(); ++i) {
               ...
               if (arg->type()->kind() == TypeKind::Class) {
                  if (!argVal->getType()->isPointerTy()) {
                     llvm::AllocaInst* tmp = Builder->CreateAlloca(arg->type()->llvmType(), nullptr, argVal->getName() + ".byval");
                     Builder->CreateStore(argVal, tmp);
                     argVal = tmp;
                  }
               } else if (expectedParamType) {
                  if (!expectedParamType->isPointerTy() && argVal->getType()->isPointerTy())
                     argVal = Builder->CreateLoad(expectedParamType, argVal, argVal->getName() + ".load");
            ...
            return Builder->CreateCall(function, args, _Type->kind() != TypeKind::Void ? funName + ".ret" : "");
         }
   ...
   llvm::Value* CallExpressionAST::codegen() {
      ...
      if (!callee) {
         utils::debugLog("{} not found: looking for C function {}", name, _callee);
      ...
      for (size_t i = 0; i < _args.size(); ++i) {
         ...
         llvm::Value* argVal = arg->codegen();
         if (!argVal) {
            utils::debugLog("arg[{}] returned nullptr", i);
            return nullptr;
         }
         ...
         if (arg->astType() == ASTType::UnaryExpression && static_cast<UnaryExpressionAST*>(arg.get())->op() == "*"sv) {
            ...
            llvm::Type* pointerValueType = derefOperand->type()->llvmType();
            if (argVal->getType()->isPointerTy() && static_cast<PointerTy*>(arg->type().get())->pointee()->kind() == TypeKind::Pointer)
               argVal = Builder->CreateLoad(pointerValueType, argVal);
         }
         ...
         if (arg->type()->kind() == TypeKind::Class) {
            if (expectedParamType && expectedParamType->isPointerTy()) {
               if (!argVal->getType()->isPointerTy()) {
                  auto tmp = Builder->CreateAlloca(arg->type()->llvmType(), nullptr, argVal->getName() + ".tmp");
                  Builder->CreateStore(argVal, tmp);
                  argVal = tmp;
               }
            } else {
               ...
               llvm::Value* refArg = argVal;
               if (!argVal->getType()->isPointerTy()) {
                  auto refTmp = Builder->CreateAlloca(arg->type()->llvmType());
                  Builder->CreateStore(argVal, refTmp);
                  refArg = refTmp;
               }
               ...
               Builder->CreateCall(copyCtor, {tmp, refArg});
               argVal = tmp;
            }
         } else if (expectedParamType) {
            if (!expectedParamType->isPointerTy() && argVal->getType()->isPointerTy())
               argVal = Builder->CreateLoad(expectedParamType, argVal);
      ...
      llvm::Value* callResult = Builder->CreateCall(callee, args, _Type->kind() != TypeKind::Void ? callee->getName() + ".ret" : "");
   ...
   llvm::Value* StaticCastAST::codegen() {
      ...
      if (castedValue->getType()->isPointerTy()) {
         ...
         if (isAddressLikeExpr) {
            ...
            castedValue = Builder->CreateLoad(sourceValueType, castedValue, castedValue->getName() + ".load");
         }
      }
   ...
   llvm::Value* ReturnStatementAST::codegen() {
      ...
      if (!returnsReference && _value->astType() == ASTType::UnaryExpression && static_cast<UnaryExpressionAST*>(_value.get())->op() == "*"sv) {
         ...
         returnValue = Builder->CreateLoad(valueTypeForCodegen(_value->type()), returnValue, returnValue->getName() + ".deref");
      }
      ...
      if (!returnsReference && _Type->kind() == TypeKind::Array) {
         ...
         if (returnValue->getType()->isPointerTy() && retType->isArrayTy()) {
            returnValue = Builder->CreateLoad(retType, returnValue, returnValue->getName() + ".load");
         }
      }
      ...
      if (_value->astType() == ASTType::MemberAccessExpression && _Type->kind() != TypeKind::Pointer) // will probably change later
         returnValue = Builder->CreateLoad(valueTypeForCodegen(_Type), returnValue, returnValue->getName() + ".load");
   ...
   llvm::Value* ExitStatement::codegen() {
      ...
      if (_value->astType() == ASTType::UnaryExpression && static_cast<UnaryExpressionAST*>(_value.get())->op() == "*"sv) {
         ...
         exitValue = Builder->CreateLoad(_value->type()->llvmType(), exitValue, exitValue->getName() + ".deref");
      }
   ...
   llvm::Function* FunctionPrototypeAST::codegen() {
      ...
      for (const auto& arg : _args) {
         if (arg->type()->kind() == TypeKind::ArgPack)
            isVarArgs = true;
         else if (arg->type()->kind() == TypeKind::Class)
   ...
   llvm::Function* FunctionAST::codegen() {
      ...
      for (auto& arg : function->args()) {
         ...
         if (DBuilder && KSDbgInfo.CurrentSubprogram && KSDbgInfo.TheFile) {
            ...
            DBuilder->insertDeclare(NamedValues[varDecl->name()],
                                    paramVar,
                                    DBuilder->createExpression(),
   ...
   llvm::Value* NewStatementAST::codegen() {
      ...
      auto emitCtorCall = [&](llvm::Value* objPtr, const std::string& className) -> llvm::Value* {
         ...
         llvm::Function* ctor = TheModule->getFunction(ctorName);
         if (!ctor)
            return LogErrorV(ctorName + " not found");

         if (objPtr->getType()->isPointerTy() &&
             allocType->kind() == TypeKind::Pointer) {
            objPtr = Builder->CreateLoad(Builder->getPtrTy(), objPtr, "obj.load");
         }
      ...
      if (isArray) {
         ...
         auto elemTy = arrTy->arrayType();

         uint64_t count = 1;

         if (arrTy->sizeExpr() &&
             arrTy->sizeExpr()->astType() == ASTType::NumberExpression) {
            auto* num = static_cast<NumberExpressionAST*>(arrTy->sizeExpr().get());
            const Number raw = num->value();

            count = std::holds_alternative<double>(raw)
                ? static_cast<uint64_t>(std::get<double>(raw))
                : std::get<uint64_t>(raw);
         }

         uint64_t elemSize = elemTy->size();
         llvm::Value* totalSize =
             Builder->getInt64(elemSize * count);

         setDebugLocationFor(this);

         ptr = Builder->CreateCall(mallocFun, {totalSize}, elemTy->string() + ".new.arr");
         // element type must be pointer element type for GEP
         llvm::Type* llvmElemTy = elemTy->llvmType();

         if (elemTy->kind() == TypeKind::Class) {
            auto className = elemTy->string();

            for (uint64_t i = 0; i < count; ++i) {
               llvm::Value* idx = Builder->getInt64(i);

               llvm::Value* elemPtr = Builder->CreateGEP(llvmElemTy, ptr, idx, className + ".new.elem");

               if (!emitCtorCall(elemPtr, className))
      ...
      uint64_t size = allocType->size();

      ptr = Builder->CreateCall(mallocFun, {Builder->getInt64(size)}, allocType->string() + ".new");

      if (allocType->kind() == TypeKind::Class) {
         auto className = allocType->string();

         if (!emitCtorCall(ptr, className))
            return LogErrorV("constructor not found for class '" + className + "'");
      } else {
         if (_args.size() > 1)
         ...
         if (!_args.empty()) {
            llvm::Value* init = _args.front()->codegen();
            if (!init) return nullptr;

            if (init->getType()->isPointerTy() &&
                _args.front()->type()->kind() != TypeKind::Pointer &&
                _args.front()->type()->kind() != TypeKind::Reference) {
               init = Builder->CreateLoad(
                   valueTypeForCodegen(_args.front()->type()),
                   init,
                   init->getName() + ".load");
            }

            llvm::Type* llvmTy = allocType->llvmType();

            init = castValueToType(init, llvmTy);

            if (init->getType() != llvmTy)
               return LogErrorV("new initializer type mismatch");
